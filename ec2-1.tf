...
echo 'export name=lkcloudtech.live' >> ~/.bashrc
echo 'export KOPS_STATE_STORE=s3://lkcloudtech.live' >> ~/.bashrc
echo 'export AWS_REGION=us-east-1' >> ~/.bashrc
echo 'export CLUSTER_NAME=lkcloudtech.live' >> ~/.bashrc
...
#   provisioner "local-exec" {
#     command = <<-EOT
#       /usr/local/bin/kops create cluster --name=kops-cluster-lkcloudtech.live \
